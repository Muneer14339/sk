...
class SmoothingConfig {
  static const int movingAverageWindow = 200; // Average last 20 points
  ...
  static const int maxTracelinePackets = 200; // ✅ NEW: Maximum packets in traceline
}

// ✅ REMOVED: TimestampedTracePoint class (no longer needed)
// Now we'll use simple packet counting

class TraceSmoothing {
  final Queue<Point> _rollBuffer = Queue();
  final Queue<Point> _pitchBuffer = Queue();
  Point? _lastSmoothedPoint;
  int _pointCounter = 0;
  ...
  Point smoothPoint(double roll, double pitch) {
    _pointCounter++;

    // Method 1: Moving Average Smoothing
    Point smoothedPoint = _applyMovingAverage(roll, pitch);

    // Method 2: Exponential Moving Average (alternative)
    // Point smoothedPoint = _applyExponentialMovingAverage(roll, pitch);
  ...
  Point _applyMovingAverage(double roll, double pitch) {
    // Add new points to buffer
    _rollBuffer.add(Point(roll, 0));
    _pitchBuffer.add(Point(pitch, 0));
    ...
    // Calculate average
    double avgRoll = _rollBuffer.map((p) => p.x).reduce((a, b) => a + b) / _rollBuffer.length;
    double avgPitch = _pitchBuffer.map((p) => p.x).reduce((a, b) => a + b) / _pitchBuffer.length;

    return Point(avgRoll, avgPitch);
  }

  Point _applyExponentialMovingAverage(double roll, double pitch) {
    if (_emaRoll == null || _emaPitch == null) {
      _emaRoll = roll;
      _emaPitch = pitch;
    } else {
    ...
    return Point(_emaRoll!, _emaPitch!);
  }

  Point _applyNoiseFilter(Point point) {
    if (_lastSmoothedPoint == null) return point;

    double distance = sqrt(
        pow(point.x - _lastSmoothedPoint!.x, 2) +
...
class BleRepositoryImpl implements BleRepository {
  ...
  void _maintainTracelinePacketLimit() {
    // Keep only the last 500 packets in the traceline
  ...
  // ✅ UPDATED: Manage shot cycle with SMOOTH trace storage + packet limit
  void _manageShotCycleWithTraceStorage(
      dynamic orientation, double roll, double pitch)
  {


    // ✅ SMOOTHING: Apply smoothing to roll/pitch
    Point smoothedPoint = _traceSmoothing.smoothPoint(roll, pitch);
    double smoothedRoll = smoothedPoint.x.toDouble();
    double smoothedPitch = smoothedPoint.y.toDouble();
    ...
    switch (_shotCycleState) {
      ...
      // ✅ SMOOTHING: Only add to trace if decimation allows AND smoothing is applied
        if (_traceSmoothing.shouldAddToTrace()) {
          _addPointToTrace(TracePoint(
              Point(smoothedRoll, smoothedPitch),
              TracePhase.preShot
          ));
          _preShotPointsCount++;
        }
      ...
      // ✅ SMOOTHING: Continue collecting post-shot data with smoothing
        ...
        if (_postShotPointsCount >= _targetPostShotPoints) {
          ...
          // ✅ Reset for next shot
          _resetForNextShot(smoothedRoll, smoothedPitch);
        } else {
  ...
  void _processCompleteShot() {
    ...
    final shotTraceData = ShotTraceData(
      ...
      analysisNotes:
      'Balanced trace: $preShotCount pre-shot + $shotCount shot + $postShotCount post-shot points (SMOOTHED + PACKET LIMITED)',
    );
  ...
  // ✅ ENHANCED: Reset for next shot with smoothing reset
  void _resetForNextShot(double roll, double pitch) {
    ...
    Point smoothedPoint = _traceSmoothing.smoothPoint(roll, pitch);
    _addPointToTrace(TracePoint(
        Point(smoothedPoint.x, smoothedPoint.y),
        TracePhase.preShot
    ));
    _preShotPointsCount = 1;
  }
  ...
  void _initializeSession() async {
    ...
    print('🔄 Session initialized - Shot cycle state: $_shotCycleState (SMOOTHING + PACKET LIMIT: ${SmoothingConfig.maxTracelinePackets})');
    ...
    bleManager.enableSensors(device).then((_) {
      ...
      _orientationSubscription =
          sensorProcessor.orientationStream.listen((orientation) {
            ...
            // ✅ ENHANCED: Shot cycle management with SMOOTH trace storage
            ...
            // Add current shot trace (limited to 500 packets)
  ...
  double _calculateStability(List<TracePoint> points) {
    ...
    final stability =
        (1 - (avgVariation / 2.0)) * 100; // Normalize to percentage
