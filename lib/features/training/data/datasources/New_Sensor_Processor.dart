...
class SensorProcessor {
  ...
  void resetForNewSession() {
    ...
    _deviceOrientation = DeviceOrientation.upward;
      ...
      SensorData accel, SensorData gyro, bool shotDetected, int timestamp)
  {
    ...
    yawFilter.update(gyro.z, 0.0012);
    if (currentDt > 0) {
      _roll = _deviceOrientation == DeviceOrientation.downward
          ? rollFilter.update(gyro.y, accelRoll, currentDt)
          : -rollFilter.update(gyro.y, accelRoll, currentDt);

      _pitch = _deviceOrientation == DeviceOrientation.downward
          ? pitchFilter.update(gyro.x, accelPitch, currentDt)
          : -pitchFilter.update(gyro.x, accelPitch, currentDt);

      _yaw = _deviceOrientation == DeviceOrientation.downward
          ? -yawFilter.estimate
          : yawFilter.estimate;
    }
    else {
  ...
  DeviceOrientation? _deviceOrientation; // ✅ nullable - detect once only
  bool _orientationLocked = false; // ✅ lock after first detection

// ✅ 2. Simplified detection (unchanged)
  DeviceOrientation _detectDeviceOrientation(double rawZ) {
    return rawZ > 0 ? DeviceOrientation.upward : DeviceOrientation.downward;
  }
  ...
  void processData(List<int> newBytes) {
    ...
    while (currentParsePosition + 3 < _dataBuffer.length) {
      if (_dataBuffer[currentParsePosition] == 0x55 &&
        ...
        // ✅ 3. Update Accelerometer processing (CRITICAL FIX)
        if (packetType == 0x08) {
          // ✅ RAW values (no sign flip yet)
          final rawAx = (x / 32768.0) * -16.0;
          final rawAy = (y / 32768.0) * -16.0;
          final rawAz = (z / 32768.0) * -16.0;

          if (!accelCalibrated) {
            // ✅ DETECT ORIENTATION ONLY ONCE (before any movement check)
            if (!_orientationLocked) {
              _deviceOrientation = _detectDeviceOrientation(rawAz);
              _orientationLocked = true;
              print('📱 Device Orientation LOCKED: ${_deviceOrientation == DeviceOrientation.upward ? "Upward ⬆️" : "Downward ⬇️"}');
            }

            // ✅ Movement check using RAW values (no sign flip)
            final diffX = (rawAx - prevAccelX).abs();
            final diffY = (rawAy - prevAccelY).abs();
            final diffZ = (rawAz - prevAccelZ).abs();
            ...
            if (totalMovement > 2) {
              print('⚠️ Movement detected ($totalMovement), resetting calibration...');
              gyroSumX = gyroSumY = gyroSumZ = 0.0;
              accelSumX = accelSumY = accelSumZ = 0.0;
              gyroCalibrationCount = accelCalibrationCount = 0;
              // ✅ DON'T reset orientation lock
            } else {
              if (accelCalibrationCount >= samplesToSkip) {
                // ✅ Apply sign flip ONLY for calibration sums
                final sign = _deviceOrientation == DeviceOrientation.downward ? 1.0 : -1.0;
                accelSumX += rawAx * sign;
                accelSumY += rawAy * sign;
                accelSumZ += rawAz * sign;
              }
            ...
            // ✅ Apply sign flip for normal operation
            final sign = _deviceOrientation == DeviceOrientation.downward ? 1.0 : -1.0;
            final ax = rawAx * sign;
            final ay = rawAy * sign;
            final az = rawAz * sign;
