...
class BleManager {
  ...
  Future<void> enableSensors(fbp.BluetoothDevice device) async {
    ...
    if (targetService == null) throw Exception("Service $serviceUuid not found");
    ...
    if (writeChar == null) throw Exception("Write characteristic $writeUuid not found");
...
// Add this method to BleManager class
  Future<Map<String, int>> readDeviceSettings(fbp.BluetoothDevice device) async {
    ...
    for (var entry in params.entries) {
      ...
      final subscription = notifyChar.onValueReceived.listen((data) {
        if (data.length >= 5 && data[0] == 0x55 && data[1] == 0xAA && data[2] == 0x00) {
