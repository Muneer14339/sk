...
class SensorProcessor {
  ...
  Orientation getOrientationRelativeToDynamicZero() {
    if (_currentShotDynamicZero == null) {
      ...
      return Orientation(_roll - rollOffset, _pitch - pitchOffset, _yaw - yawOffset, false,
          DateTime.now().millisecondsSinceEpoch);
    }
    ...
    final relativeYaw = _yaw - yawOffset;
  ...
  void processSensorPair(
    ...
    if (!isRecalibrated && !lastCalibrated && (now - lastShotTime!.millisecondsSinceEpoch) >= 300) {
    ...
    if (isStable(rollList, 0.2) == false || isStable(pitchList, 0.2) == false) {
      ...
      if ((((_roll - rollOffset) * 180 / pi).abs() > 90) ||
        ...
        if (isStableForRecalibration(rollList, 01) == true &&
            isStableForRecalibration(pitchList, 01) == true) {
      ...
      if (_dataBuffer[currentParsePosition] == 0x55 &&
        ...
        if (packetType == 0x06) {
          ...
          if (lastShotTime == null ||
              now.difference(lastShotTime!).inMilliseconds >= 2000 && (now.millisecondsSinceEpoch - lastCalibratedTime!) >= 300) {
