...
class SensorProcessor {
  ...
  void processSensorPair(
    ...
    if (rollList.length >= 1000) {
      rollList.removeAt(0);
    }
    if (pitchList.length >= 1000) {
      pitchList.removeAt(0);
    }
    rollList.add((_roll - rollOffset) * 180 / pi);
    pitchList.add((_pitch - rollOffset) * 180 / pi);
    // ✅ NEW: Use dynamic zero point if available, otherwise fall back to static calibration
    Orientation orientation;
    if (isStable(rollList, 0.2)==false || isStable(pitchList, 0.2)==false) {
      // recalibrate();
      // Use dynamic zero point for this shot
      orientation = Orientation(
        _roll - rollOffset ,
        _pitch - pitchOffset,
        0, // yaw not used
    ...
    //   // Use dynamic zero point for this shot
    //   orientation = Orientation(
    //     _roll - rollOffset - _currentShotDynamicZero!.roll,
    //     _pitch - pitchOffset - _currentShotDynamicZero!.pitch,
    //     0, // yaw not used
    //     shotDetected,
    //     timestamp,
    //   );
