...
class Orientation {
  ...
  Orientation(this.roll, this.pitch, this.yaw, this.shotDetected, this.timestamp);
}
...
class SensorProcessor {
  ...
  // ✅ NEW: Shot Detection Event Stream
  final StreamController<ShotDetectedEvent> _shotDetectionController =
  StreamController<ShotDetectedEvent>.broadcast();
  Stream<ShotDetectedEvent> get shotDetectionStream => _shotDetectionController.stream;
  ...
  Stream<Orientation> get orientationStream => orientationStreamController.stream;

  // ✅ NEW: Calculate magnitude helper
  double _calculateMagnitude(double x, double y, double z) {
    return sqrt(x * x + y * y + z * z);
  }

  // ✅ NEW: Process accelerometer data with shot analysis
  void processAccelerometerData(double x, double y, double z, double cant, double tilt) {
    ...
    if (_isStabilityPeriodValid()) {
      ...
      if (_lastShotTime == null ||
          now.difference(_lastShotTime!).inMilliseconds >= _minTimeBetweenShots) {
  ...
  bool _isStabilityPeriodValid() {
    ...
    final avgMagnitude = recentData.map((r) => r.magnitude).reduce((a, b) => a + b) / recentData.length;
    final variance = recentData.map((r) => pow(r.magnitude - avgMagnitude, 2)).reduce((a, b) => a + b) / recentData.length;
  ...
  void _processShotFromBuffer(double threshold) {
    ...
    for (int i = 0; i < _stabilityBuffer.length; i++) {
      ...
      if (current.magnitude > threshold) {
        ...
        print("🔫 Shot #$_shotCounter Detected: ${shotReading.magnitude.toStringAsFixed(2)}g at ${shotReading.timestamp}");
        ...
        final recoveryTime = postShot.isNotEmpty && recoveryAchieved
            ? postShot.last.timestamp.difference(shotReading.timestamp).inMilliseconds
            : (postShot.isNotEmpty
            ? postShot.last.timestamp.difference(shotReading.timestamp).inMilliseconds
            : 1000);
        ...
        final shotEvent = ShotDetectedEvent(
          ...
          isValidShot: true,
          reason: 'Shot detected with ${shotReading.magnitude.toStringAsFixed(2)}g spike',
          analysis: analysis,
        );
      ...
      )
  {
    try {
      ...
      final shotDetectionMetrics = {
        'magnitude': shotReading.magnitude,
        'detectionTime': shotReading.timestamp.millisecondsSinceEpoch.toDouble(),
      ...
      final postShotMetrics = _calculatePostShotMetrics(postShotReadings, recoveryTimeMs);
      ...
      return ShotAnalysis(
        ...
        analysisNotes: 'Shot detected at ${shotReading.magnitude.toStringAsFixed(2)}g with ${recoveryTimeMs}ms recovery time',
      );
  ...
  Map<String, double> _calculatePreShotMetrics(List<SensorReading> readings) {
    ...
    final drift = sqrt(
        pow(lastReading.cant - firstReading.cant, 2) +
    ...
    return {
      ...
      'stability': (1 - sqrt(variance)) * 100, // Convert to stability percentage
      'drift': drift,
  ...
  Map<String, double> _calculatePostShotMetrics(List<SensorReading> readings, int recoveryTimeMs) {
  ...
  void processSensorPair(SensorData accel, SensorData gyro, bool shotDetected, int timestamp) {
    double accelPitch = atan2(accel.y, accel.z);
    double accelRoll = atan2(-accel.x, sqrt(accel.y * accel.y + accel.z * accel.z));
    ...
    Orientation orientation = Orientation(
        _roll - rollOffset, _pitch - pitchOffset, 0, shotDetected, timestamp);
  ...
  void processData(List<int> newBytes) {
    ...
    while (currentParsePosition + 3 < _dataBuffer.length) {
      if (_dataBuffer[currentParsePosition] == 0x55 &&
        ...
        if (packetType == 0x06) {
          final now = DateTime.now();
          if (lastShotTime == null || now.difference(lastShotTime!).inMilliseconds >= 2000) {
            // 2 seconds have passed since last shot
            shotDetectedInThisPacket = true;
            lastShotTime = now; // update the time
            print('[SensorProcessor] SHOT DETECTED (edge)!');
          }
        }

        if (!((packetType == 0x08 || packetType == 0x0A) && payloadLength == 0x06)) {
        ...
        if (packetType == 0x08) {
          ...
          if (!accelCalibrated) {
            ...
            if (totalMovement > 3.0) {
              ...
              if (debug) print('Accelerometer moved significantly, resetting calibration.');
            } else {
              ...
              if (accelCalibrationCount >= calibrationSamples + samplesToSkip) {
                ...
                if (debug) print("Accelerometer calibrated: X=${accelXOffset.toStringAsFixed(4)}, Y=${accelYOffset.toStringAsFixed(4)}, Z=${accelZOffset.toStringAsFixed(4)}");
              }
            }
          } else {
            ...
            processAccelerometerData(filteredAccelX, filteredAccelY, filteredAccelZ,
                _roll * 180 / pi, _pitch * 180 / pi);

            latestAccelData = SensorData(filteredAccelX, filteredAccelY, filteredAccelZ);
        ...
        // Process Gyroscope Data
          ...
          if (!gyroCalibrated) {
            ...
            if (gyroCalibrationCount >= calibrationSamples + samplesToSkip) {
              ...
              gyroCalibrated = true;
              if (debug) print("Gyroscope calibrated: X=${gyroXOffset.toStringAsFixed(4)}, Y=${gyroYOffset.toStringAsFixed(4)}");
            }
          } else {
        ...
        if (gyroCalibrated && accelCalibrated &&
            latestAccelData != null && latestGyroData != null &&
            latestAccelTimestamp != null && latestGyroTimestamp != null) {

          int consolidatedTimestamp = max(latestAccelTimestamp!, latestGyroTimestamp!);
