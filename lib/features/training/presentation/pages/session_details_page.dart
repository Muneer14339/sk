...
class _SessionDetailPageViewState extends State<_SessionDetailPageView> {
  ...
  void _resumeAnimation() {
    if (_isPaused && _lastSelectedShotId != -1) {
      ...
      _startTraceAnimation(context, _lastSelectedShotId,
          context.read<TrainingSessionBloc>().state,
          resume: true);
    }
  }
    ...
    return Scaffold(
      ...
      body: BlocBuilder<TrainingSessionBloc, TrainingSessionState>(
        builder: (context, trainingState) {
          return BlocBuilder<SessionDetailsBloc, SessionDetailsState>(
            builder: (context, state) {
              if (state is SessionDetailsLoading) {
                ...
                return _buildLoadedContent(context, state, trainingState);
  ...
  Widget _buildLoadedContent(BuildContext context, SessionDetailsLoaded state,
      TrainingSessionState trainingState) {
    return Stack(
      children: [
        Column(
          children: [
            Expanded(
              child: SingleChildScrollView(
                ...
                child: Column(
                  ...
                  children: [
                    _buildSessionHeaderCard(context, state, trainingState),
                    const SizedBox(height: 20),
                    _buildViewTabs(context, state),
                    const SizedBox(height: 20),
                    _buildViewContent(context, state, trainingState),
                  ...
                  child: Center(
                    child: GestureDetector(
                      ...
                      child: Container(
                        ...
                        child:
                            _buildTargetDisplay(context, state, trainingState),
                      ),
                    ),
                  ),
                ),
  ...
  Widget _buildSessionHeaderCard(BuildContext context,
      SessionDetailsLoaded state, TrainingSessionState trainingState) {
    final sessionDetails = state.sessionDetails;

    // Get actual shot count from training session
    final actualShotCount = trainingState.steadinessShots.isNotEmpty
        ? trainingState.steadinessShots.length
  ...
  Widget _buildViewContent(BuildContext context, SessionDetailsLoaded state,
    ...
    switch (state.currentView) {
      ...
      case 'shots':
        return _buildShotAnalysisContent(context, state, trainingState);
      case 'timeline':
        return _buildTimelineContent(context, state, trainingState);
      default:
        return Container();
    }
  }
  ...
  Widget _buildShotAnalysisContent(BuildContext context,
      SessionDetailsLoaded state, TrainingSessionState trainingState) {
    // Get actual steadiness shots from training session
    final steadinessShots = trainingState.steadinessShots;

    // Debug print
    print('🔍 Shot Analysis: Found ${steadinessShots.length} steadiness shots');

    // Use actual steadiness shots if available, otherwise fallback to session details
    final shots = steadinessShots.isNotEmpty
        ? steadinessShots.map((shot) {
            return ShotDetailsEntity(
              ...
              metrics: shot.metrics.map((key, value) {
                if (value is num) {
              ...
              hasTraceData: shot.tracelinePoints.isNotEmpty,
            );
          }).toList()
    ...
    return Column(
      crossAxisAlignment: CrossAxisAlignment.stretch,
      children: [
        _buildShotSelectionRow(context, state, trainingState),
        const SizedBox(height: 15),
        _buildTraceStatusIndicator(context, state, trainingState),
        const SizedBox(height: 15),
        _buildTargetDisplay(context, state, trainingState),
        const SizedBox(height: 15),
        _buildShotInfoPanel(context, selectedShot, state, trainingState),
      ],
    );
  }

  Widget _buildShotSelectionRow(BuildContext context,
      SessionDetailsLoaded state, TrainingSessionState trainingState) {
    // Get actual steadiness shots from training session
    final steadinessShots = trainingState.steadinessShots;
    ...
    // Debug print
    print(
        '🔍 Session Details: Found ${steadinessShots.length} steadiness shots');
    for (var shot in steadinessShots) {
      print(
          '   - Shot ${shot.shotNumber}: Score ${shot.score}, Position (${shot.position.dx.toStringAsFixed(1)}, ${shot.position.dy.toStringAsFixed(1)})');
    }

    // Use actual steadiness shots if available, otherwise fallback to empty list
    final shots = steadinessShots.isNotEmpty
        ? steadinessShots.map((shot) {
            return ShotDetailsEntity(
              id: shot.shotNumber,
                ...
                return Container(
                  ...
                  child: GestureDetector(
                    onTap: () {
                      ...
                      _startTraceAnimation(context, shot.id, trainingState,
                          restart: true);
                    },
                    child: Container(
                      width: 50,
  ...
  Widget _buildTraceStatusIndicator(BuildContext context,
      SessionDetailsLoaded state, TrainingSessionState trainingState) {
    final selectedShotId = state.selectedShotId;
    final hasTraceData = selectedShotId != null &&
        _getActualTracePoints(context, selectedShotId, trainingState)
            .isNotEmpty;
      ...
      child: Row(
        children: [
          ...
          Expanded(
            child: Text(
              hasTraceData
                  ? '🎯 Shot #$selectedShotId traceline is displayed below with ${_getActualTracePoints(context, selectedShotId, trainingState).length}/${_getTotalPointsForShot(context, selectedShotId, trainingState)} trace packets${_currentAnimatedPoints > 0 ? ' (animating...)' : ''}'
                  : selectedShotId != null
  ...
  Widget _buildTargetDisplay(BuildContext context, SessionDetailsLoaded state,
    ...
    if (state.selectedShotId != null) {
      points =
          _getActualTracePoints(context, state.selectedShotId!, trainingState);
            ...
            child: Container(
              decoration: BoxDecoration(
                border: Border.all(
                    color: state.selectedShotId != null &&
                            _getActualTracePoints(context,
                                    state.selectedShotId!, trainingState)
                                .isNotEmpty
                        ? const Color(0xff17a2b8)
              ...
              child: Stack(
                ...
                children: [
                  ...
                  _zoomEnabled
                      ? zoomAbleTargetRigns(state, trainingState)
                      : _buildTargetRings(state, trainingState),
                  if (state.selectedShotId != null)
                    Positioned(
                        top: 5,
                        left: 5,
                        child: Container(
                            ...
                            child: Text(
                                'Trace: Pre($preShotCount) + Shot($shotCount) + Post($postShotCount) packets',
                                style: const TextStyle(
                                    color: Color(0xFF17A2B8),
                                    fontSize: 10,
                                    fontWeight: FontWeight.bold)))),
          ...
          if (state.selectedShotId != null)
            Positioned.fill(
                child: Container(
                    ...
                    child: CustomPaint(
                        painter: TracePainter(_getActualTracePoints(
                            context, state.selectedShotId!, trainingState))))),
          // Play/Pause controls
          if (state.selectedShotId != null)
            Positioned(
              bottom: 20,
              ...
              child: Column(
                ...
                children: [
                  ...
                  Container(
                    ...
                    child: Text(
                      '$_currentAnimatedPoints/${_getTotalPointsForShot(context, state.selectedShotId!, trainingState)}',
                      style: const TextStyle(
                        color: Color(0xff2c3e50),
                        fontSize: 12,
    ...
    return StatefulBuilder(
      builder: (context, setState) {
        return InteractiveViewer(
          ...
          child: _buildTargetRings(state, trainingState),
        );
      },
    );
  }

  List<streaming.TracePoint> _convertTracePoints(
      List<streaming.TracePoint> tracePoints) {
    // Convert streaming TracePoints to streaming TracePoints for TracePainter
    return tracePoints.map((point) {
      return streaming.TracePoint(
        Point3D(point.point.x, point.point.y, point.point.z),
        point.phase,
      );
    }).toList();
  }

  // Get actual trace points from steadiness shot data
  List<streaming.TracePoint> _getActualTracePoints(
      BuildContext context, int shotId, TrainingSessionState trainingState) {
    final steadinessShots = trainingState.steadinessShots;

    // Find the steadiness shot for the selected shot
    final steadinessShot =
        steadinessShots.where((shot) => shot.shotNumber == shotId).firstOrNull;

    if (steadinessShot != null && steadinessShot.tracelinePoints.isNotEmpty) {
      // Convert SteadinessShotData trace points to streaming TracePoints
      final allPoints = steadinessShot.tracelinePoints.map((point) {
        return streaming.TracePoint(
            Point3D(point.point.x, point.point.y, point.point.z), point.phase);
      }).toList();
  ...
  void _startTraceAnimation(
    ...
    int totalPoints = 0;
    if (steadinessShot != null && steadinessShot.tracelinePoints.isNotEmpty) {
      totalPoints = steadinessShot.tracelinePoints.length;
    }
  ...
  int _getTotalPointsForShot(
    ...
    if (steadinessShot != null && steadinessShot.tracelinePoints.isNotEmpty) {
      return steadinessShot.tracelinePoints.length;
    } else {
      return 0;
    }
  }
  ...
  Widget _buildShotInfoPanel(BuildContext context, dynamic selectedShot,
      SessionDetailsLoaded state, TrainingSessionState trainingState) {
            ...
            Container(
              ...
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 8),
                  Text(
                    '🎯 Complete traceline stored from training session with ${_getActualTracePoints(context, selectedShot.id, trainingState).length}/${_getTotalPointsForShot(context, selectedShot.id, trainingState)} data packets${_currentAnimatedPoints > 0 ? ' (animating...)' : ''}',
                    style: const TextStyle(
                      fontSize: 13,
                      fontWeight: FontWeight.w500,
                      color: Color(0xff28a745),
  ...
  Widget _buildTimelineContent(BuildContext context, SessionDetailsLoaded state,
      TrainingSessionState trainingState) {
    // Get actual steadiness shots from training session
    final steadinessShots = trainingState.steadinessShots;
