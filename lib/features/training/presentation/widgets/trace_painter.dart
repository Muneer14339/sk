...
/// Enhanced TracePainter for smooth gun movement visualization with current position marker
///
/// This painter creates smooth, responsive trace lines that follow the gun's movement
/// in real-time. It includes several advanced features:
///
/// 🔧 **Smoothing**: Reduces sensor jitter for cleaner lines
/// 📈 **Interpolation**: Adds intermediate points for smoother curves
/// 🚀 **Velocity-based Smoothing**: Adapts smoothing based on movement speed
/// 🎨 **Adaptive Thickness**: Line thickness varies with movement velocity
/// 🎯 **Current Position Marker**: Shows the current head of the trace line
///
/// The trace line automatically adapts to different movement phases:
/// - 🔵 Blue: Older pre-shot movements (stable periods)
/// - 🟡 Yellow: Recent pre-shot movements (last 0.15s)
/// - 🔴 Red: Shot detection and post-shot recovery
/// - 🎯 Green: Current position marker (head of trace)
///
/// Usage:
/// ```dart
/// TracePainter(
///   points,
///   smoothingFactor: 0.25,        // Lower = more responsive
///   enableInterpolation: true,     // Smoother curves
///   interpolationPoints: 2,        // Performance vs smoothness trade-off
///   enableVelocityBasedSmoothing: true,  // Adaptive smoothing
///   enableAdaptiveThickness: true,       // Dynamic line thickness
///   showCurrentPositionMarker: true,     // Show current position dot
/// )
/// ```
class TracePainter extends CustomPainter {
  ...
  final double movementDampening; // Controls how much movement is dampened
  ...
  // Performance optimization: Cache processed points by phase
  ...
  TracePainter(
      this.points, {
        ...
        this.showCurrentPositionMarker = true, // NEW: Default enabled
        this.animateCurrentMarker = true, // NEW: Default animated
        this.currentMarkerSize = 8.0, // NEW: Default marker size
      });
  ...
  void paint(Canvas canvas, Size size) {
    ...
    const double highlightSeconds =
    0.15; // Reduced from 5s to 0.15s for better responsiveness
    ...
    if (shotPoints.isNotEmpty) {
      ...
      // Get last N preShot points before first shot (recent movement)
    ...
    if (bluePreShot.isNotEmpty) {
      _drawSmoothPhaseLines(
          ...
          size,
          bluePreShot,
          const Color(0xFF17A2B8).withValues(alpha: 0.8), // Blue
          2,
          'Pre-shot-Blue');
    }
    ...
    if (yellowPreShot.isNotEmpty) {
      _drawSmoothPhaseLines(
          ...
          size,
          yellowPreShot,
          Colors.amber.withValues(alpha: 0.9), // Yellow
          2.5,
          'Pre-shot-Yellow');
    }
    ...
    if (shotPoints.isNotEmpty) {
      _drawSmoothPhaseLines(
          ...
          size,
          shotPoints,
          const Color(0xFFDC3545).withValues(alpha: 0.9), // Bright Red
          2.3,
          'Shot');
    }
    ...
    if (postShotPoints.isNotEmpty) {
      _drawSmoothPhaseLines(
          ...
          size,
          postShotPoints,
          const Color(0xFFDC3545).withValues(alpha: 0.7), // Light Red
          2,
          'Post-shot');
    }

    // Draw enhanced shot markers
    _drawEnhancedShotMarkers(canvas, size, shotPoints);

    // 🎯 NEW: Draw current position marker (head of trace line)
  ...
  void _drawCurrentPositionMarker(Canvas canvas, Size size, TracePoint currentPoint) {
    ...
    final distanceFromCenter = math.sqrt(math.pow(x - centerX, 2) + math.pow(y - centerY, 2));

    if (distanceFromCenter > maxRadius) return; // Don't draw if outside target area
    ...
    if (animateCurrentMarker) {
      ...
      animationValue = 0.7 + 0.3 * math.sin(now / 200.0); // Pulse between 0.7 and 1.0
    }
    ...
    switch (currentPoint.phase) {
      case TracePhase.preShot:
        markerColor = const Color(0xFF28A745); // Green for active movement
        glowColor = const Color(0xFF28A745).withValues(alpha: 0.4);
        break;
      case TracePhase.shot:
        markerColor = const Color(0xFFFFD700); // Gold for shot
        glowColor = const Color(0xFFFFD700).withValues(alpha: 0.6);
        break;
      case TracePhase.postShot:
        markerColor = const Color(0xFFFF6B35); // Orange for post-shot
        glowColor = const Color(0xFFFF6B35).withValues(alpha: 0.4);
        break;
    }

    final markerRadius = currentMarkerSize * 0.4;
  ...
  void _drawDirectionalIndicator(Canvas canvas, double x, double y, double radius, Color color) {
    ...
    final headX1 = endX - arrowHeadLength * math.cos(math.atan2(screenDirY, screenDirX) - arrowAngle);
    final headY1 = endY - arrowHeadLength * math.sin(math.atan2(screenDirY, screenDirX) - arrowAngle);
    final headX2 = endX - arrowHeadLength * math.cos(math.atan2(screenDirY, screenDirX) + arrowAngle);
    final headY2 = endY - arrowHeadLength * math.sin(math.atan2(screenDirY, screenDirX) + arrowAngle);
    ...
    for (int i = 1; i < points.length - 1; i++) {
      ...
      if (distance1 < maxAllowedMovement && distance2 < maxAllowedMovement) {
        ...
        filtered.add(TracePoint(
            math.Point(interpolatedX, interpolatedY), points[i].phase));
      }
    }
  ...
  List<TracePoint> _applySmoothing(List<TracePoint> points) {
    ...
    for (int i = 1; i < points.length - 1; i++) {
      ...
      smoothed.add(TracePoint(math.Point(dampedX, dampedY), points[i].phase));
    }
  ...
  List<TracePoint> _applyVelocityBasedSmoothing(List<TracePoint> points) {
    ...
    for (int i = 1; i < points.length - 1; i++) {
      ...
      velocitySmoothed
          .add(TracePoint(math.Point(dampedX, dampedY), points[i].phase));
    }
    ...
    for (int i = 0; i < points.length - 1; i++) {
      ...
      for (int j = 1; j <= interpolationCount; j++) {
        ...
        interpolated.add(TracePoint(
            math.Point(interpolatedX, interpolatedY), current.phase));
      }
    }
  ...
  void _drawEnhancedShotMarkers(
    ...
    for (var tp in shotPoints) {
      ...
      final glowPaint = Paint()
        ..color = const Color(0xFFDC3545).withValues(alpha: 0.4)
        ..style = PaintingStyle.fill;
      ...
      canvas.drawCircle(Offset(x, y), 6.0, markerPaint);
