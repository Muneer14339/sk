...
class TrainingSessionBloc
  ...
  // void _onSimulateShot(SimulateShot event, Emitter<TrainingSessionState> emit) {
  //   final random = Random();
  //   final double normalizedX = random.nextDouble() * 2 - 1;
  //   final double normalizedY = random.nextDouble() * 2 - 1;
  //   final double distance = sqrt(normalizedX * normalizedX + normalizedY * normalizedY);

  //   ShotAccuracy accuracy;
  //   if (distance < 0.2) {
  //     accuracy = ShotAccuracy.excellent;
  //   } else if (distance < 0.4) {
  //     accuracy = ShotAccuracy.good;
  //   } else if (distance < 0.6) {
  //     accuracy = ShotAccuracy.fair;
  //   } else {
  //     accuracy = ShotAccuracy.poor;
  //   }

  //   int newShotCount = state.shotCount;
  //   if (event.streamingModel?.shotDetected ?? false) {
  //     newShotCount++;
  //   }

  //   final newShotData = List<ShotData>.from(state.shotData)
  //     ..add(ShotData(normalizedX, normalizedY, accuracy));

  //   // ✅ FIXED: Check for session completion but don't immediately complete
  //   if (newShotCount >= (state.program?.noOfShots ?? 0)) {
  //     print('[TrainingSessionBloc] Last shot detected. Waiting for trace data...');
  //     _waitingForLastShotTrace = true;
  //     _targetShotCount = newShotCount;

  //     emit(state.copyWith(
  //         shotData: newShotData,
  //         shotCount: newShotCount,
  //         sessionCompleted: false, // ✅ Don't complete yet
  //         isTraining: true, // ✅ Keep training active
  //         aiFeedbackText: "Last shot detected! Collecting complete trace data..."));
  //   } else {
  //     print('[TrainingSessionBloc] Shot detected! Analyzing your trace...');
  //     emit(state.copyWith(
  //         shotData: newShotData,
  //         shotCount: newShotCount,
  //         aiFeedbackText: "Shot detected! Analyzing your trace..."));
  //   }
  // }
  ...
  Future<void> _onEnableSensors(
    ...
    // ✅ ENHANCED: Reset shot traces for new sensor session
    _sessionShotTraces.clear();
    _lastShotDetected = false;
    _waitingForLastShotTrace = false;
    ...
    // ✅ NEW: Listen to shot traces stream from repository
    ...
    await emit.forEach<StreamingModel>(
      ...
      onData: (streamingModel) {
        ...
        if (shotEdge) {
          print(
              '[TrainingSessionBloc] Shot detected: ${streamingModel.shotDetected}');
          print('[TrainingSessionBloc] SHOT DETECTED (edge)!');

          // ✅ ENHANCED: Generate shot analysis feedback with trace data
          String analysisText = _generateShotAnalysisFeedback(streamingModel);

          // Simulate shot logic (copy from _onSimulateShot)
          // final random = Random();
          // final double normalizedX = random.nextDouble() * 2 - 1;
          // final double normalizedY = random.nextDouble() * 2 - 1;
          ...
          // ✅ FIXED: Don't immediately complete session when last shot detected
          if (newShotCount >= _targetShotCount) {
            print(
                '[TrainingSessionBloc] Last shot detected. Waiting for complete trace data...');
            _waitingForLastShotTrace = true;
            ...
            print(
                '[TrainingSessionBloc] Shot detected! Analyzing your trace...');
            newState = newState.copyWith(
                // shotData: newShotData,
                shotCount: newShotCount,
                aiFeedbackText: analysisText);
          }
        }
      ...
      onError: (error, stackTrace) {
        print('[TrainingSessionBloc] sensorStream error: $error');
  ...
  Future<void> _onDisableSensors(
    ...
    print('[TrainingSessionBloc] sensorStreamSubscription cancelled');
