...
// 🔧 UPDATED CONSTANTS - Shot Analysis Only
  ...
  static const int SHOT_ANALYSIS_LOOKBACK = 62;   // 1 second at ~60Hz for analysis
  static const int POST_SHOT_PACKETS = 0;        // Fixed: was -23 (bug)
  static const int SHOT_LOOKBACK_POINTS = 50;     // Unchanged - shot point detection
  ...
  // NEW: Pause handler - data retain karta hai
  // MODIFY: _onPauseTrainingSession
      ...
      PauseTrainingSession event, Emitter<TrainingSessionState> emit)
  {
    ...
    emit(state.copyWith(
      isTraining: false,
      isPaused: true,
      pausedDuration: currentDuration, // NEW: Save current duration
      pauseStartTime: DateTime.now(), // NEW: Mark pause time
    ));
...
// MODIFY: _onResumeTrainingSession
  ...
  {
    // Only stop if in training mode (not calibration)
    ...
    emit(state.copyWith(
      ...
      pausedDuration: null, // NEW: Clear
      pauseStartTime: null, // NEW: Clear
      waitingForLastShotTrace: false,
  ...
  {
    // Only stop if in calibration mode
    if (!state.isCalibrationMode) return;
    ...
    if (shotEdgeUI) {
      if (state.isCalibrationMode) {
        ...
        if (!newVisGate || updatedTracelineBuffer.length <= 50) {
          final nextShotNumber = state.shotCount + state.missedShotCount + 1; // NEW
          add(IncrementMissedShot(shotNumber: nextShotNumber)); // NEW: Pass shot number
        } else {
          add(const DetectShotEvent());
        }
      }
  ...
  void _onDetectShotEvent(
    ...
    // ✅ NEW: Use SHOT_ANALYSIS_LOOKBACK (62 packets = ~1 second)
    if (state.tracelineBuffer.length >= SHOT_ANALYSIS_LOOKBACK) {
      final lookbackIndex = state.tracelineBuffer.length - SHOT_ANALYSIS_LOOKBACK;
    ...
    emit(state.copyWith(
      ...
      postShotStartIndex: math.max(0, state.tracePoints.length - actualLookback),
    ));
...
// 3. Update _onContinuePostShotCapture
      ...
      ContinuePostShotCapture event, Emitter<TrainingSessionState> emit)
  {
    ...
    if (newExistingPostShotPoints >= POST_SHOT_PACKETS) {
      print('🎯 Post-shot capture for shot $shotNumber complete. Final count: $newExistingPostShotPoints');
      add(CompleteShotCapture(completeBuffer, shotNumber)); // NEW: Pass shot number
      return;
    }
...
// Replace _onEnableSensors method:

  // In training_session_bloc.dart - Replace _onEnableSensors method
    ...
    try {
      ...
      final dataTimeout = Timer(const Duration(seconds: 5), () {
        if (!receivedFirstData && state.isSensorsEnabled) {
          emit(state.copyWith(
            isSensorsEnabled: false,
            sensorError: 'Sensor timeout: No data received. Please reconnect and try again.',
            isTraining: false,
          ));
        }
      });
  ...
  // 2. Update _checkSessionCompletion method (around line 565)
      ...
      Emitter<TrainingSessionState> emit, List<ShotTraceData> shotTraces)
  {
    ...
    // UPDATED: Check total shots (detected + missed)
  ...
  // In training_session_bloc.dart - Update _saveCurrentSession method
